...
//   table.appendChild(tableBody);
  ...
  const hierarchy = (data) => {
    ...
    var tree = hierarchy(data.tables.DEFAULT.rows)

    console.log(tree)

    function add_node(root_el, node){
        var node_el = document.createElement('li')
        node_el.innerHTML = `<span class="caret">${node[2]}</span>`
        node_el.onclick = clicked
        root_el.appendChild(node_el)
        var child_holder = document.createElement('ul')
        child_holder.className = "nested"
        // console.log(`Add ${node[2]} to ${root_el.innerHTML}`)
        node_el.appendChild(child_holder)
        node.children.forEach( (child) => {
            // let child_el = document.createElement('li')
            // child_el.innerHTML = child[2]
            // child_holder.appendChild(child_el)
            add_node(child_holder, child)
        } )
    }


    var tree_container = document.createElement('ul')
    container.appendChild(tree_container)
    tree_container.id = 'myUL'
    var node = tree[0]
    add_node(tree_container, node)

//   const script = document.createElement("script");
//     script.innerHTML = `var toggler = document.getElementsByClassName("caret");
// var i;
...
//   script.src = "https://storage.googleapis.com/looker_custom_viz_plukon/hierarchy_tree/onclick.js";
